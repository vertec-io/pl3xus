    ...
    view! {
        <div class="bg-background rounded border border-border/8 relative">
            ...
            <div class="absolute top-1.5 right-1.5 flex gap-1 z-10">
                // Refresh button
                <button
                    class="p-0.5 hover:bg-border/6 rounded"
                    title="Refresh I/O"
                    on:click={
            ...
            // Collapsible header
            <button
                class="w-full flex items-center justify-between p-2 pr-12 hover:bg-border/4 transition-colors"
    ...
    view! {
        <div
            class=move || format!(
                ...
                if value.get() { "bg-success/20 text-success" } else { "bg-border/4 text-muted-foreground" }
            )
    ...
    view! {
        <button
            class=move || format!(
                ...
                if value.get() { "bg-warning/20 text-warning hover:bg-warning/30" } else { "bg-border/4 text-muted-foreground hover:bg-border/6" }
            )
...
fn AnalogIndicator(
    ...
    view! {
        <div
            class="flex flex-col items-center justify-center p-1 rounded text-[8px] bg-border/4"
...
fn AnalogOutput(
    ...
    view! {
        <div
            class="flex flex-col items-center justify-center p-1 rounded text-[8px] bg-warning/10 cursor-pointer hover:bg-warning/20"
