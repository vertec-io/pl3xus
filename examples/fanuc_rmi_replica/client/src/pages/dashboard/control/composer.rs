...
pub fn CommandComposerModal() -> impl IntoView {
    ...
    // Form state
    let (command_type, set_command_type) = signal("linear".to_string());
...
fn ComposerForm(
    ...
    let is_joint = Memo::new(move |_| command_type.get() == "joint");

    view! {
        // Command Type Selection
        <div class="space-y-2">
            <label class="text-[10px] text-[#888888] uppercase tracking-wide">"Command Type"</label>
            <div class="flex gap-2">
                {["linear", "joint", "relative"].into_iter().map(|t| {
                    let label = match t {
                        "linear" => "Linear (L)",
                        "joint" => "Joint (J)",
                        "relative" => "Relative",
                    ...
                    view! {
                        <button
                            class=move || format!(
                                "flex-1 text-[10px] py-2 rounded border transition-colors {}",
                                if command_type.get() == t {
                                    "bg-[#00d9ff20] border-[#00d9ff] text-[#00d9ff]"
                                } else {
                                    "bg-[#111111] border-[#ffffff08] text-[#888888] hover:bg-[#1a1a1a]"
                                }
                            )
                            on:click=move |_| set_command_type.set(t.to_string())
                        >
                            {label}
                        </button>
                    }
        ...
        <div class="space-y-2">
            <div class="flex items-center justify-between">
                <label class="text-[10px] text-[#888888] uppercase tracking-wide">
                    {move || if is_joint.get() { "Joint Angles" } else { "Position" }}
                </label>
                <button
            ...
            <div class="grid grid-cols-6 gap-2">
                <PositionInput label=move || if is_joint.get() { "J1" } else { "X" } value=x set_value=set_x unit=move || if is_joint.get() { "°" } else { "mm" }/>
                <PositionInput label=move || if is_joint.get() { "J2" } else { "Y" } value=y set_value=set_y unit=move || if is_joint.get() { "°" } else { "mm" }/>
                <PositionInput label=move || if is_joint.get() { "J3" } else { "Z" } value=z set_value=set_z unit=move || if is_joint.get() { "°" } else { "mm" }/>
                <PositionInput label=move || if is_joint.get() { "J4" } else { "W" } value=w set_value=set_w unit=move || if is_joint.get() { "°" } else { "°" }/>
                <PositionInput label=move || if is_joint.get() { "J5" } else { "P" } value=p set_value=set_p unit=move || if is_joint.get() { "°" } else { "°" }/>
                <PositionInput label=move || if is_joint.get() { "J6" } else { "R" } value=r set_value=set_r unit=move || if is_joint.get() { "°" } else { "°" }/>
            </div>
        </div>
        ...
        <div class="space-y-1">
            ...
            <div class="flex">
                <input
                    type="number"
                    step="0.1"
                    class="flex-1 min-w-0 bg-[#111111] border border-[#ffffff08] rounded-l px-1 py-1 text-[10px] text-white focus:border-[#00d9ff] focus:outline-none text-center"
