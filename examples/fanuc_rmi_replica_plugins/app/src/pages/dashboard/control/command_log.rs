                    ...
                    <div class="flex gap-1">
                        <button
                            class=move || if active_tab.get() == "messages" {
                                "text-[9px] px-2 py-0.5 bg-[#00d9ff20] text-primary border border-primary rounded"
                            } else {
                                "text-[9px] px-2 py-0.5 bg-[#ffffff05] text-muted-foreground border border-border/15 rounded hover:bg-border/10"
                            }
                        ...
                        <button
                            class=move || {
                                ...
                                let base_class = if active_tab.get() == "errors" {
                                    "text-[9px] px-2 py-0.5 bg-destructive/15 text-destructive border border-destructive rounded"
                                } else {
                                    "text-[9px] px-2 py-0.5 bg-[#ffffff05] text-muted-foreground border border-border/15 rounded hover:bg-border/10"
                                };
                                if error_count > 0 {
                        ...
                        >
                            ...
                            {move || {
                                ...
                                if count > 0 {
                                    Some(view! {
                                        <span class="ml-1 bg-destructive text-primary-foreground text-[8px] px-1 rounded-full font-bold">
                                            {count}
                                        </span>
                                    })
                                } else {
