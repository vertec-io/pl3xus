    ...
    for request in requests.read() {
        ...
        let programs = db.as_ref()
            .and_then(|db| {
                let conn = db.connection();
                let conn = conn.lock().unwrap();
                database::list_programs(&conn).ok()
            })
            .unwrap_or_default();
    ...
    for request in requests.read() {
        ...
        let program = db.as_ref()
            .and_then(|db| {
                let conn = db.connection();
                let conn = conn.lock().unwrap();
                database::get_program(&conn, program_id).ok()
            })
            .flatten();
    ...
    for request in requests.read() {
        ...
        let result = db.as_ref()
            .map(|db| {
                let conn = db.connection();
                let conn = conn.lock().unwrap();
                database::create_program(&conn, &inner.name, inner.description.as_deref())
            })
            .unwrap_or(Err(anyhow::anyhow!("Database not available")));
    ...
    for request in requests.read() {
        ...
        let result = db.as_ref()
            .map(|db| {
                let conn = db.connection();
                let conn = conn.lock().unwrap();
                database::delete_program(&conn, program_id)
            })
            .unwrap_or(Err(anyhow::anyhow!("Database not available")));
    ...
    for request in requests.read() {
        ...
        let result = db.as_ref()
            ...
            .and_then(|db| {
                let conn = db.connection();
                let conn = conn.lock().unwrap();
                database::update_program_settings(
                    &conn,
                    req.program_id,
                    req.start_x,
                    req.start_y,
...
fn parse_and_insert_csv(
    ...
    let db_res = db.as_ref().ok_or_else(|| anyhow::anyhow!("Database not available"))?;
    ...
    database::insert_instructions(&conn, program_id, &instructions)?;
    ...
    if start_x.is_some() || end_x.is_some() || move_speed.is_some() {
        if let Err(e) = database::update_program_settings(
            &conn,
            program_id,
...
fn handle_load_program(
    ...
    for request in requests.read() {
        ...
        let conn = db_res.connection();
        let conn = conn.lock().unwrap();
        match database::get_program(&conn, program_id) {
    ...
    for request in requests.read() {
        ...
        let (success, error) = match db.as_ref() {
            Some(db_res) => {
                let conn = db_res.connection();
                let conn = conn.lock().unwrap();
                match database::reset_database(&conn) {
                    Ok(_) => (true, None),
                    Err(e) => (false, Some(e.to_string())),
                }
            }
