...
use crate::types::{BufferDisplayData, ExecutionState, Pause, Resume, Start, Stop, Subsystems};
...
/// - FANUC: `robot_hmi_plugins::RobotPlugin` (includes motion handler)
/// - Duet: `robot_hmi_duet::DuetPlugin`
...
impl Plugin for ExecutionPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.requests::<(Start, Pause, Resume, Stop), WebSocketProvider>()
            .targeted()
        ...
        app.add_systems(
            Update,
            (handle_start, handle_pause, handle_resume, handle_stop),
        );
        ...
        app.configure_sets(Update, SubsystemValidation.before(coordinate_validation));
