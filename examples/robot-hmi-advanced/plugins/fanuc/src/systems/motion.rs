...
use robot_hmi_execution::types::{BufferState, MotionType};
use robot_hmi_execution::systems::{DeviceStatus, MotionCommandEvent};
    ...
    match (old_category, new_category) {
        ...
        (STATE_EXECUTING | STATE_PAUSED, STATE_STOPPED) => {
            ...
            tokio_runtime.spawn_background_task(move |_ctx| async move {
                // abort the motion
                let _ = driver.0.abort().await;
                // run the startup sequence again
                let _ = driver.0.startup_sequence().await;
            });
        }
