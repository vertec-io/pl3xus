...
This document outlines the steps to migrate program management from `robot_hmi_fanuc` to `robot_hmi_programs` and eliminate legacy execution code from the fanuc crate.
...
|-------|----------------------|
| `robot_hmi_programs` | New crate - types defined but handlers are placeholders |
| `robot_hmi_fanuc` | Program CRUD handlers + legacy execution state + motion handlers |
| `robot_hmi_execution` | Orchestrator, ToolpathBuffer, BufferState, DeviceStatus |
...
2. Create new schema in `robot_hmi_programs/src/database.rs`:
...
**Goal:** Remove legacy execution state from fanuc crate. Confirm these are no longer being registered/used in the robot_hmi_advanced actual compiled application ad anything that is still being used migrate to the execution plugin (should be complete but need to confirm)
