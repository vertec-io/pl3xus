...
| `current_state_analysis.md` | Detailed analysis of pl3xus/robot-hmi-advanced current state |
...
- **Application**: `examples/robot-hmi` - A FANUC RMI web control replica
- **Robot HMI Advanced**: `plugins/` - An existing robotics application using pl3xus patterns

## Key Files to Examine

### Current Position Representations (robot-hmi)
```
examples/shared/robot_hmi_types/src/lib.rs
...
### Robot HMI Advanced's Approach (the pattern to evaluate)
```
...
```
examples/robot-hmi/server/src/plugins/program.rs
...
**Should pl3xus/robot-hmi adopt Isometry3<f32> (SE3) as the generalized coordinate type?**

### Robot HMI Advanced's Pattern
```rust
...
3. **Configuration Separation Pattern**
   - Robot HMI Advanced separates "where to go" (Isometry3) from "how to get there" (Configuration)
...
- `examples/robot-hmi/research/ORCHESTRATOR_TECHNICAL_SPEC.md`
...
```bash
# View the robot-hmi-advanced toolpath system
view plugins/src/toolpath/

# View current position types
view examples/shared/robot_hmi_types/src/lib.rs

# View motion packet generation
view examples/robot-hmi/server/src/plugins/program.rs
...
This only works because robot-hmi-advanced's toolpaths assume a fixed tool-down orientation. The fix is documented in `quaternion_to_euler.md`.
...
# View how robot-hmi-advanced sends toolpath points
view plugins/src/printer/systems/toolpath_execution.rs

# View robot-hmi's position handling
view examples/robot-hmi/server/src/plugins/program.rs
