...
To:
```rust
app.add_plugins(robot_hmi_core::CorePlugin);
app.add_plugins(robot_hmi_fanuc::FanucPlugin);
app.add_plugins(robot_hmi_execution::ExecutionPlugin);
...
```bash
# Check compilation
cd examples/robot_hmi_advanced
cargo check --package robot_hmi_fanuc
...
# See warnings count
cargo check --package robot_hmi_fanuc 2>&1 | grep "warning:" | wc -l
```
