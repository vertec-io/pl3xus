...
Refactor `examples/robot_hmi_advanced/` from the current structure to multi-crate plugins.

## Current State

```
examples/robot_hmi_advanced/
...
3. Update package name to `robot_hmi_robotics` (keep same)
...
6. Update package name to `robot_hmi_execution` (keep same)
...
```toml
robot_hmi_core = { path = "../plugins/core", default-features = false }
robot_hmi_robotics = { path = "../plugins/robotics", default-features = false }
```
