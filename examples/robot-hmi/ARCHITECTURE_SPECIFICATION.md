...
replica: https://github.com/vertec-io/pl3xus/tree/main/examples/robot-hmi --> locally available at /home/apino/dev/pl3xus/examples/robot-hmi (this repo)
...
Take special care to understand this framework so that you can find anti-patterns and violations of the pl3xus framework in our curent robot-hmi implementation. We need to fix these anti-patterns. Many of these anti-patterns are documented below, along with known issues, and a clarified architecture specification.
...
Also review the other previous research attempts in /research/active (soon to be archived) under robot_hmi_completion (written by previous agents and used to achieve the current implementation). This previous research has allowed us to make good progress, but we've taken and accumulated technical debt as a result of not fully understanding the pl3xus framework, not following the core philosophy written below, and not referencing the original fanuc_rmi_api application thoroughly enough.
...
// In robot_hmi_types/src/lib.rs (shared crate):
...
### Types MUST be in `robot_hmi_types` (Shared Crate)
...
use robot_hmi_types::ProgramLineInfo;
```
...
- `ProgramLine` in context.rs vs `ProgramLineInfo` in robot_hmi_types
...
- Define shared types in `robot_hmi_types`
...
| File | Issue | Action |
|------|-------|--------|
| `robot_hmi_types/src/lib.rs` | ExecutionState fields | Verify all needed fields present |
