...
This document outlines the implementation plan for refactoring the robot-hmi client to follow the server-authoritative architecture specified in `ARCHITECTURE_SPECIFICATION.md`.
...
**Fix:** Use `ProgramLineInfo` from `robot_hmi_types` everywhere.
...
2. Update imports to use `ProgramLineInfo` from `robot_hmi_types`
...
use robot_hmi_types::ExecutionState;
...
- ✅ All code now uses `ProgramLineInfo` from `robot_hmi_types`
...
1. Start the server: `cargo run -p robot_hmi_server`
