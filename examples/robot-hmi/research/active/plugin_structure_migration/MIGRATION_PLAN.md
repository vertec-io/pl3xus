...
First, identify the domains in robot-hmi to structure plugins:
...
[dependencies]
robot-hmi_plugins = { path = "../plugins", default-features = false, features = ["stores"] }
...
- [ ] `cargo build -p robot-hmi_plugins --features ecs,server`
- [ ] `cargo build -p robot-hmi_plugins --features stores --no-default-features`
- [ ] `cargo build -p robot-hmi_server`
- [ ] `cargo build -p robot-hmi_app --target wasm32-unknown-unknown`
