...
// Client matches against std::any::type_name::<GetProgram>() → "robot_hmi_types::GetProgram"
...
```bash
# Check compilation
cargo check -p robot_hmi_client
cargo check -p robot_hmi_server
...
# Full build
cargo build --release -p robot_hmi_server
cd examples/robot-hmi/client && trunk build --release
```
