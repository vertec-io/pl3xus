...
The **robot-hmi** is a reference implementation demonstrating the pl3xus framework (Bevy ECS server + Leptos WASM client). It replicates the original Fanuc_RMI_API web application functionality.
...
├── examples/
│   └── robot-hmi/         # Primary example application
│       ├── client/                # Leptos WASM client
│       ├── server/                # Bevy ECS server
│       └── research/active/       # Research documents
└── examples/shared/
    └── robot_hmi_types/       # Shared message/component types
```
...
### Example Application (robot-hmi)

| File | Purpose |
|------|---------|
| `examples/robot-hmi/server/src/main.rs` | Server setup and plugin registration |
| `examples/robot-hmi/server/src/plugins/` | Server plugins (connection, requests, sync, etc.) |
| `examples/robot-hmi/client/src/app.rs` | Client setup and component registration |
| `examples/robot-hmi/client/src/layout/top_bar.rs` | Connection UI, quick settings |
| `examples/robot-hmi/client/src/pages/dashboard/context.rs` | `SystemEntityContext` for entity IDs |
...
4. **Test after each change** - `cargo check --package robot_hmi_server --package robot_hmi_client`
